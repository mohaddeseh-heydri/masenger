...
function verifyform() {
    ...
    //تکرار رمز
    var pw_repeat = document.getElementById("key_repeat").value;
    var errorKey_repeat = document.getElementById("text-error-key-repeat");
    if (pw != pw_repeat) {
        errorKey_repeat.innerHTML = "تکرار رمز اشتباه است";
        document.querySelector(".error-key-repeat").style.display = 'block';
        return false;
    } else {
        document.querySelector(".error-key-repeat").style.display = 'none';
        kr = true;
        ramz = pw_repeat;
    }
...
const ke_repeat = document.getElementById("key_repeat");
ke_repeat.addEventListener("input", function (e) {
    rsult = verifyform();
    if (rsult) {
        button.classList.remove("btn");
        document.querySelector(".error-signup").style.display = 'none';
